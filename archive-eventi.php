...
?>
	...
	<div class="container mx-auto px-4">
		...
		<div class="lg:hidden custom-taxonomy-select mb-6" data-taxonomy-filter="true" data-archive-url="<?php echo esc_attr($archive_url_eventi); ?>">
			<button type="button"
					class="custom-taxonomy-select__button w-full px-4 py-2 desc-1 rounded-full border-0 cursor-pointer focus:outline-none focus:ring-2 focus:ring-primary-400 flex items-center justify-between <?php echo ($current_cat_id > 0) ? 'bg-secondary-500 text-white' : 'bg-gray-200'; ?>"
			...
			<div class="custom-taxonomy-select__dropdown hidden absolute w-full mt-2 bg-white rounded-lg shadow-lg border border-gray-200 max-h-60 overflow-y-auto">
				<a href="<?php echo esc_url($all_url); ?>"
				   class="custom-taxonomy-select__option block px-4 py-2 desc-1 no-underline transition-colors <?php echo ($current_cat_id == 0) ? 'bg-secondary-500 text-white' : 'text-gray-700 hover:bg-gray-100'; ?>">
				...
				<a href="<?php echo esc_url($cat_url); ?>"
				   class="custom-taxonomy-select__option block px-4 py-2 desc-1 no-underline transition-colors <?php echo ($current_cat_id == $cat->term_id) ? 'bg-secondary-500 text-white' : 'text-gray-700 hover:bg-gray-100'; ?>">
		...
		<div class="hidden lg:flex gap-2 flex-wrap mb-8" data-aos="fade">
			...
			<?php $cat_url = $stato === 'passate' ? add_query_arg( 'stato', 'passate', get_term_link($cat) ) : get_term_link($cat); ?>
			<a href="<?php echo esc_url($cat_url); ?>"
			   class="desc-1 px-6 py-2 rounded-full transition-colors no-underline whitespace-nowrap <?php echo ($current_cat_id == $cat->term_id) ? 'bg-secondary-500 text-white hover:bg-primary-500' : 'bg-gray-200 hover:bg-secondary-500 hover:text-white'; ?>">
				<?php echo esc_html($cat->name); ?>
			</a>
			<?php endforeach; ?>
		</div>
