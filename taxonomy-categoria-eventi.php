...
<div class="container mx-auto px-4">
	...
	<div class="lg:hidden custom-taxonomy-select mb-6" data-taxonomy-filter="true" data-archive-url="<?php echo esc_attr($archive_url_eventi); ?>">
		<button type="button"
				class="custom-taxonomy-select__button w-full px-4 py-2 desc-1 rounded-full border-0 cursor-pointer focus:outline-none focus:ring-2 focus:ring-primary-400 flex items-center justify-between bg-primary-400 text-white"
		...
		<div class="custom-taxonomy-select__dropdown hidden absolute w-full mt-2 bg-white rounded-lg shadow-lg border border-gray-200 max-h-60 overflow-y-auto">
			...
			<a href="<?php echo esc_url($cat_url); ?>"
			   class="custom-taxonomy-select__option block px-4 py-2 desc-1 no-underline transition-colors <?php echo ($cat->term_id == $term->term_id) ? 'bg-primary-400 text-white' : 'text-gray-700 hover:bg-gray-100'; ?>">
	...
	<div class="hidden lg:flex gap-2 flex-wrap mb-8" data-aos="fade-up">
		<a href="<?php echo esc_url($all_url); ?>"
		   class="desc-1 px-6 py-2 rounded-full transition-colors no-underline whitespace-nowrap bg-gray-200 hover:bg-primary-400 hover:text-white">
			<?php _e('Tutte le categorie','mongolfiera'); ?>
		</a>
		<?php foreach ($categorie_eventi as $cat) : ?>
		<?php $cat_url = $stato === 'passate' ? add_query_arg( 'stato', 'passate', get_term_link($cat) ) : get_term_link($cat); ?>
		<a href="<?php echo esc_url($cat_url); ?>"
		   class="desc-1 px-6 py-2 rounded-full transition-colors no-underline whitespace-nowrap <?php echo ($cat->term_id == $term->term_id) ? 'bg-primary-400 text-white hover:bg-primary-500' : 'bg-gray-200 hover:bg-primary-400 hover:text-white'; ?>">
			<?php echo esc_html($cat->name); ?>
		</a>
		<?php endforeach; ?>
	</div>
	...
	<div class="my-6 mb-12 lg:my-12 flex justify-center" data-aos="fade-up">
		<?php if ( $stato === 'passate' ) : ?>
			<a href="<?php echo esc_url( get_term_link($term) ); ?>" class="btn btn-primary-light"><?php echo esc_html( $term->name ); ?> <?php _e('attivi','mongolfiera'); ?></a>
		<?php else : ?>
			<a href="<?php echo esc_url( add_query_arg( 'stato', 'passate', get_term_link($term) ) ); ?>" class="btn btn-primary-light"><?php echo esc_html( $term->name ); ?> <?php _e('passati','mongolfiera'); ?></a>
		<?php endif; ?>
	</div>
</div>
<?php endif; ?>
